...
from django.db.models import Q
import openai
from .models import ChatMessage, Product  # Import ChatMessage và Product
from django.conf import settings
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                recent_messages = ChatMessage.objects.filter(user=user).order_by('-created_at')[:5]
                messages = [
                    {"role": "system", "content": """Bạn là một chuyên gia tư vấn thời trang. Nhiệm vụ của bạn:
                    1. Tư vấn chọn size quần áo dựa trên chiều cao, cân nặng
                    2. Gợi ý phối đồ phù hợp với dịp, phong cách
                    3. Tư vấn chọn màu sắc phù hợp với tone da
                    4. Giới thiệu các xu hướng thời trang mới
                    5. Trả lời các câu hỏi về chất liệu, bảo quản
                ...
                product_keywords = ["sản phẩm", "quần", "áo", "giày", "túi", "phụ kiện", "thời trang"]
                has_product_keywords = any(keyword in message.lower() for keyword in product_keywords)
                
                # If the message is product-related, try to find relevant products
                if has_product_keywords:
                    logger.info("Message contains product keywords, searching products...")
                    product_response = process_chat_message(message)
                    if product_response.get('success') and product_response.get('products'):
                        logger.info(f"Found {len(product_response['products'])} relevant products")
                        return JsonResponse({
                            'success': True,
                            'message': ai_response,
                        ...
                        # If no products found, still return the AI response
                        return JsonResponse({
                            'success': True,
                ...
                return JsonResponse({
                    'success': False,
                    'message': 'Lỗi xác thực API. Vui lòng kiểm tra lại API key của OpenAI.'
                })
            except openai.error.RateLimitError as e:
                logger.error(f"OpenAI API Rate Limit Error: {str(e)}")
                return JsonResponse({
                    'success': False,
                    'message': 'Hệ thống OpenAI đang quá tải. Vui lòng thử lại sau 1-2 phút.'
                })
            except openai.error.APIError as e:
                logger.error(f"OpenAI API Error: {str(e)}")
                return JsonResponse({
                    'success': False,
                    'message': f'Lỗi từ OpenAI API: {str(e)}. Vui lòng thử lại sau.'
                })
            except Exception as e:
                logger.error(f"Unexpected error when calling OpenAI API: {str(e)}")
                return JsonResponse({
                    'success': False,
                    'message': f'Có lỗi không mong muốn xảy ra khi gọi API: {str(e)}. Vui lòng thử lại sau.'
                })
            ...
            logger.error(f"Unexpected error in chatgpt_message view: {str(e)}")
            return JsonResponse({
                'success': False,
                'message': f'Có lỗi xảy ra trong hệ thống: {str(e)}. Vui lòng thử lại sau.'
            })
...
def process_chat_message(message):
    ...
    # Tạo truy vấn tìm kiếm dựa trên tên sản phẩm
    ...
    try:
        ...
        logger.error(f"Lỗi truy vấn sản phẩm: {str(e)}")
        return {
            'success': False,
            'message': 'Có lỗi xảy ra khi tìm kiếm sản phẩm. Vui lòng thử lại sau.'
        }

    if products.exists():
        product_data_list = []
        for product in products:
            rating_display = '★' * int(round(product.rating))
            rating_display += '☆' * (5 - int(round(product.rating)))

            product_data = {
                'id': product.id,
                'name': product.name,
                'price': str(product.display_price),
                'image': product.image.url if product.image else '/static/images/No_images.png',
                ...
                'original_price': str(product.original_price) if product.sale_price else None
            }
            product_data_list.append(product_data)

        return {
            'success': True,
            'message': f'Tôi tìm thấy {len(product_data_list)} sản phẩm phù hợp với yêu cầu của bạn:',
            'products': product_data_list
        }
    else:
        return {
            'success': True,
            'message': 'Xin lỗi, hiện tại chúng tôi không tìm thấy sản phẩm phù hợp với yêu cầu của bạn. Bạn có thể thử tìm với từ khóa khác.'
        }
